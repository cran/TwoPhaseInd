mele <- function(data,			## dataset (data.frame)
                 ...
                 difffactor=0.0000001	## 
                 ##maxit=1000,		## max iteration
                 ##verbose=FALSE		## TRUE to turn on debug log
                 ){
  ## source("/home/ting/proj/jdai/2phi/pkg/TwoPhaseInd/R/estlik_fun.R")
  ...
  if(ind){
    ...
    while (diff > tol) {
      ...
      betanew <- beta + solve(out$cheese) %*% out$score
    ...
    d1 <- NXY4/(NXY2+NXY4)*(Wbar[from.y==1 & from.x==1,] - matrix(m1,nrow=nxy,ncol=p,byrow=T))
    nxy <- sum(from.y==1 & from.x==0)
    d2 <- NXY3/(NXY1+NXY3)*(Wbar[from.y==1 & from.x==0,] - matrix(m1,nrow=nxy,ncol=p,byrow=T))
    ...
    d3 <- NXY2/(NXY2+NXY4)*(Wbar[from.y==0 & from.x==1,] - matrix(m1,nrow=nxy,ncol=p,byrow=T))
    nxy <- sum(from.y==0 & from.x==0)
    d4 <- NXY1/(NXY1+NXY3)*(Wbar[from.y==0 & from.x==0,] - matrix(m1,nrow=nxy,ncol=p,byrow=T))
    I2 <- t(rbind(d3,d4)) %*% (rbind(d3,d4))
    I <- I1 + I2
    stder <- round(sqrt(diag(solve(cheese) %*% (cheese+I) %*% solve(cheese))),4)
