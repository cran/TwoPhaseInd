...
void profile_NR_noind(
  ...
  long i,j,k,l,*Nyx, nx0, nx1, n_cov, nz0, nz1, n11=0, n00=0, n01=0, n10=0, count1=0, count2=0,count,it, itt;
  FILE *file; 
 ...
 if (*verbose==1) fprintf(file,"\nn00=%i\n\n", n00);
  ...
  /* this is the rough NR where covariate distribution does not get updated */  

  diff=1;
  itt=0;
  while (diff > pow(10,-5) & itt < *maxit) {
	...
	if (y1[i]==0) fit1[i]=1-fit1[i]; 
      ...
      if (*verbose==1) print_vector_double(fit1,*n_subject*2,file);
	...
	if (y[i]==0) fit[i]=1-fit[i]; 
     ...
     if (*verbose==1) print_vector_double(err,*n_subject,file);
     if (*verbose==1) print_vector_double(fit,*n_subject,file);
	...
	if (x1[i]==0)  pzx[i]= pzx[i]*2/pzxsum0;  else  pzx[i]= pzx[i]*2/pzxsum1;
      ...
      if (*verbose==1) print_vector_long(NXYcount,4,file);
      if (*verbose==1) print_vector_double(tt,*n_subject*2,file);
      if (*verbose==1) print_vector_long(Nyx,*n_subject*2,file);
	...
	for (j=0;j<2;j++) {
	  for (k=0;k<n_cov;k++) ss[j][k]=0.0;
	    ...
	    if (y[k]==0) fit[k]=1-fit[k]; 
          ...
          if (*verbose==1) print_vector_double(err,*n_subject,file);
          if (*verbose==1) print_vector_double(err1,*n_subject*2,file);
          ...
          if (*verbose==1)    print_vector_double(tt,*n_subject*2,file);
          if (*verbose==1)    print_vector_double(pzx,*n_subject*2,file);
          if (*verbose==1)    fprintf(file,"\nthe sum of pzx 1 = %8.9f",pzxsum1); 
          if (*verbose==1)    fprintf(file,"\nthe sum of pzx 0 = %8.9f",pzxsum0);
	     ...
	     if (k<*n_subject) {
                  ...
                  wgt1[k]=Nyx[k-*n_subject]*fit1[k-*n_subject]*pzx[k-*n_subject]/pyx[k-*n_subject];
	...
	if (*verbose==1) print_matrix_double(ss,2,n_cov,file);
      ...
      if (*verbose==1)  print_vector_double(infmatvec,count,file);
      ...
      if (*verbose==1)  {
             ...
             print_matrix_double(infmatinv,n_cov,n_cov,file);
      }
      for (i=0;i<n_cov;i++) {
	    change=0.0;
            for (j=0;j<n_cov;j++) {  
	...
	betavec[i]=fabs(beta[i]-betanew[i]); 
        if (diff<betavec[i]) diff=betavec[i];
        beta[i]=betanew[i];
      }
     if (*verbose==1) print_vector_double(betavec,n_cov,file); 
  ...
  /* now start to update covariate distribution in each cycle */
	...
	it++;
	  ...
	  pzxdiffvec[i]=fabs(pzx[i]-pzxnew[i]); 
          if (dif<pzxdiffvec[i]) dif=pzxdiffvec[i];
          pzx[i]=pzxnew[i];
        }
          ...
          for (k=0;k < *n_subject*2;k++) {
	       ...
	       it++;
	         ...
	         pzxdiffvec[k]=fabs(pzxnew[k]-pzx[k]); 
                 if (dif<pzxdiffvec[k]) dif=pzxdiffvec[k];
                 pzx[k]=pzxnew[k];
               }
            }
	...
	betavec[i]=fabs(beta[i]-betanew[i]); 
     ...
     for (i=0;i<*n_subject;i++) free(X[i]); free(X);
     for (i=0;i<*n_subject*2;i++) free(X1[i]); free(X1);
     for (i=0;i<n_cov;i++) free(infmat[i]); free(infmat);     
     for (i=0;i<2;i++) free(ss[i]); free(ss);
     for (i=0;i<n_cov;i++) free(infmatinv[i]); free(infmatinv);  
...
void profile_NR_ind(
  ...
  long i,j,k,l,N,n_cov,count,*Nyx, n11=0, n00=0, n01=0, n10=0, count1=0, count2=0,itt,it;
  FILE *file; 
  file = fopen("debug.txt", "w"); 
 ...
 if (*verbose==1) { 
  ...
  while (diff > pow(10,-5) & itt < *maxit) {
	...
	if (y[i]==0) fit[i]=1-fit[i]; 
      ...
      if (*verbose==1) print_vector_double(err1,*n_subject*4,file);
      if (*verbose==1) print_vector_double(err,*n_subject,file);
      pzsum=0.0;
      for (i=0;i < *n_subject*4;i++) {
	...
	pz[i]= pz[i]*4/pzsum;  
      ...
      if (*verbose==1) print_vector_double(pz,*n_subject*4,file);   if (*verbose==1) print_vector_double(tt,*n_subject*4,file);
      for   (j=0;j<n_cov;j++) score[j]=0.0;     
	...
	if (i<*n_subject) {
         ...
         if (*verbose==1) print_vector_double(wgt1,*n_subject*5,file);
         if (*verbose==1) print_vector_double(score,n_cov,file);
      /* now start numerical differentiation */
	...
	for (j=0;j<2;j++) {
	  for (k=0;k<n_cov;k++) ss[j][k]=0.0;
	    ...
	    if (y[k]==0) fit[k]=1-fit[k]; 
          ...
          if (*verbose==1) print_vector_double(fit1,*n_subject*4,file);  
          for (k=0;k < *n_subject*4;k++) {
	     ...
	     if (k<*n_subject) {
                  ...
                  wgt1[k]=Nyx[k-*n_subject]*fit1[k-*n_subject]*pz[k-*n_subject]/pyx[k-*n_subject];
                  for (l=0;l<n_cov;l++) ss[j][l]+= X1[k-*n_subject][l]*wgt1[k]*err1[k-*n_subject]; 
             }
          }
            if (*verbose==1) print_vector_double(wgt1,*n_subject*5,file);
  ...
  while (diff > pow(10,-6) & itt < 20) {
	...
	it++;
	  ...
	  pzdiffvec[i]=fabs(pz[i]-pznew[i]); 
          if (dif<pzdiffvec[i]) dif=pzdiffvec[i];
          pz[i]=pznew[i];
        }
	     ...
	     it++;
	      ...
	      if (k<*n_subject) pznew[k] = wgt1[k] + wgt1[k+*n_subject] + wgt1[k+*n_subject*2] + wgt1[k+*n_subject*3] + wgt1[k+*n_subject*4];
	       ...
	       pzdiffvec[k]=fabs(pz[k]-pznew[k]); 
               if (dif<pzdiffvec[k]) dif=pzdiffvec[k];
               pz[k]=pznew[k];
           }
	  }
	...
	betavec[i]=fabs(beta[i]-betanew[i]); 
     ...
     for (i=0;i<*n_subject;i++) free(X[i]); free(X);
     for (i=0;i<*n_subject*4;i++) free(X1[i]); free(X1);
     for (i=0;i<n_cov;i++) free(infmat[i]); free(infmat);     
     for (i=0;i<2;i++) free(ss[i]); free(ss);
     for (i=0;i<n_cov;i++) free(infmatinv[i]); free(infmatinv);  
